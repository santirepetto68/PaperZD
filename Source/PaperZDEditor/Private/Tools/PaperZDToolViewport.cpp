...
struct FZoomLevelEntry
{
    ...
    : DisplayText(FText::Format(NSLOCTEXT("PaperZD", "Zoom", "Zoom {0}"), InDisplayText))
...
static const FZoomLevelEntry& GetZoomLevel(int Index)
{
    ...
    {
        FZoomLevelEntry(0.03125f, NSLOCTEXT("PaperZD", "ZoomLevel_1_32", "1:32")),
        FZoomLevelEntry(0.0625f, NSLOCTEXT("PaperZD", "ZoomLevel_1_16", "1:16")),
        FZoomLevelEntry(0.125f, NSLOCTEXT("PaperZD", "ZoomLevel_1_8", "1:8")),
        FZoomLevelEntry(0.250f, NSLOCTEXT("PaperZD", "ZoomLevel_1_4", "1:4")),
        FZoomLevelEntry(0.500f, NSLOCTEXT("PaperZD", "ZoomLevel_1_2", "1:2")),
        FZoomLevelEntry(0.750f, NSLOCTEXT("PaperZD", "ZoomLevel_3_4", "3:4")),
        FZoomLevelEntry(0.875f, NSLOCTEXT("PaperZD", "ZoomLevel_7_8", "7:8")),
        FZoomLevelEntry(1.000f, NSLOCTEXT("PaperZD", "ZoomLevel_1_1", "1:1")),
        FZoomLevelEntry(2.000f, NSLOCTEXT("PaperZD", "ZoomLevel_2x", "2x")),
        FZoomLevelEntry(3.000f, NSLOCTEXT("PaperZD", "ZoomLevel_3x", "3x")),
        FZoomLevelEntry(4.500f, NSLOCTEXT("PaperZD", "ZoomLevel_4x", "4x")),
        FZoomLevelEntry(5.000f, NSLOCTEXT("PaperZD", "ZoomLevel_5x", "5x")),
        FZoomLevelEntry(6.000f, NSLOCTEXT("PaperZD", "ZoomLevel_6x", "6x")),
        FZoomLevelEntry(7.000f, NSLOCTEXT("PaperZD", "ZoomLevel_7x", "7x")),
        FZoomLevelEntry(8.000f, NSLOCTEXT("PaperZD", "ZoomLevel_8x", "8x")),
        FZoomLevelEntry(16.000f, NSLOCTEXT("PaperZD", "ZoomLevel_16x", "16x")),
        FZoomLevelEntry(32.000f, NSLOCTEXT("PaperZD", "ZoomLevel_32x", "32x"))
    };
    return ZoomLevels[Index];
}

//////////////////////////////////////////////////////////////////////////
// SPaperZDToolViewport

SPaperZDToolViewport::~SPaperZDToolViewport()
{
    ViewportClient.Reset();
}

TSharedRef<FEditorViewportClient> SPaperZDToolViewport::MakeEditorViewportClient()
{
    return ViewportClient.ToSharedRef();
}

void SPaperZDToolViewport::Construct(const FArguments& InArgs, TSharedRef<FPaperZDToolViewportClient> InViewportClient)
{
    ...
    [
        SNew(STextBlock)
        .TextStyle(FPaperZDEditorStyle::Get(), "PaperZDEditor.Tools.ViewportZoomTextStyle")
        .Text(this, &SPaperZDToolViewport::GetZoomText)
        .ColorAndOpacity(this, &SPaperZDToolViewport::GetZoomTextColorAndOpacity)
    ];
    ...
    [
        SNew(SBorder)
        .BorderImage(FPaperZDEditorStyle::Get().GetBrush("PaperZDEditor.Tools.ViewportTitleBackground"))
        .HAlign(HAlign_Fill)
        ...
        [
            ...
            [
                ...
                [
                    SNew(STextBlock)
                    .TextStyle(FPaperZDEditorStyle::Get(), "PaperZDEditor.Tools.ViewportTitleTextStyle")
                    .Text(this, &SPaperZDToolViewport::GetTitleText)
                ]
            ]
        ]
    ];
...
void SPaperZDToolViewport::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
...
FReply SPaperZDToolViewport::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
    TotalMouseDelta = 0;
...
FReply SPaperZDToolViewport::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SPaperZDToolViewport::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SPaperZDToolViewport::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FCursorReply SPaperZDToolViewport::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
...
int32 SPaperZDToolViewport::FindNearestZoomLevel(float InZoomAmount, bool bRoundDown) const
{
...
float SPaperZDToolViewport::GetZoomAmount() const
{
    return GetZoomLevel(ZoomLevel).ZoomAmount;
}

FText SPaperZDToolViewport::GetZoomText() const
{
    return GetZoomLevel(ZoomLevel).DisplayText;
}

FSlateColor SPaperZDToolViewport::GetZoomTextColorAndOpacity() const
{
    return FLinearColor(1, 1, 1, 1.25f - ZoomLevelFade.GetLerp() * 0.75f);
}

FVector2D SPaperZDToolViewport::GetViewOffset() const
{
    return ViewOffset;
}

FVector2D SPaperZDToolViewport::ComputeEdgePanAmount(const FGeometry& MyGeometry, const FVector2D& TargetPosition)
{
...
void SPaperZDToolViewport::UpdateViewOffset(const FGeometry& MyGeometry, const FVector2D& TargetPosition)
{
    const FVector2D PanAmount = ComputeEdgePanAmount(MyGeometry, TargetPosition) / GetZoomAmount();
    ViewOffset += PanAmount;
}

void SPaperZDToolViewport::RequestDeferredPan(const FVector2D& UpdatePosition)
{
    bRequestDeferredPan = true;
    DeferredPanPosition = UpdatePosition;
}

FVector2D SPaperZDToolViewport::GraphCoordToPanelCoord(const FVector2D& GraphSpaceCoordinate) const
{
    return (GraphSpaceCoordinate - GetViewOffset()) * GetZoomAmount();
}

FVector2D SPaperZDToolViewport::PanelCoordToGraphCoord(const FVector2D& PanelSpaceCoordinate) const
{
    return PanelSpaceCoordinate / GetZoomAmount() + GetViewOffset();
}

FSlateRect SPaperZDToolViewport::PanelRectToGraphRect(const FSlateRect& PanelSpaceRect) const
{
...
void SPaperZDToolViewport::PaintSoftwareCursor(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 DrawLayerId) const
{
...
int32 SPaperZDToolViewport::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
FText SPaperZDToolViewport::GetTitleText() const
{
    return NSLOCTEXT("PaperZD", "TileSetPaletteTitle", "palette");
}
